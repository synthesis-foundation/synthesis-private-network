...
Note that Synthesis will automatically populate sane defaults for any
...
// options that are necessary for an Synthesis node to run. You will need to
// supply one of these structs to the Synthesis core when starting a node.
type NodeConfig struct {
	...
	AdminListen         string                     `comment:"Listen address for admin connections. Default is to listen for local\nconnections either on TCP/9001 or a UNIX socket depending on your\nplatform. Use this value for synthesisctl -endpoint=X. To disable\nthe admin socket, use the value \"none\" instead."`
	...
	NodeInfoPrivacy     bool                       `comment:"By default, nodeinfo contains some defaults including the platform,\narchitecture and Synthesis version. These can help when surveying\nthe network and diagnosing network routing problems. Enabling\nnodeinfo privacy prevents this, so that only items specified in\n\"NodeInfo\" are sent back if specified."`
