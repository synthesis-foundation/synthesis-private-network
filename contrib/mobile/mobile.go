...
// Synthesis mobile package is meant to "plug the gap" for mobile support, as
...
func (m *Synthesis) StartAutoconfigure() error {
	return m.StartJSON([]byte("{}"))
}

// StartJSON starts a node with the given JSON config. You can get JSON config
// (rather than HJSON) by using the GenerateConfigJSON() function
func (m *Synthesis) StartJSON(configjson []byte) error {
	...
	// Setup the Synthesis node itself.
	{
...
// Send sends a packet to Synthesis. It should be a fully formed
// IPv6 packet
func (m *Synthesis) Send(p []byte) error {
	if m.iprwc == nil {
		return nil
	}
	_, _ = m.iprwc.Write(p)
...
// Send sends a packet from given buffer to Synthesis. From first byte up to length.
func (m *Synthesis) SendBuffer(p []byte, length int) error {
	if m.iprwc == nil {
		return nil
	}
	if len(p) < length {
...
// Recv waits for and reads a packet coming from Synthesis. It
// will be a fully formed IPv6 packet
func (m *Synthesis) Recv() ([]byte, error) {
	if m.iprwc == nil {
		return nil, nil
	}
	var buf [65535]byte
...
// Recv waits for and reads a packet coming from Synthesis to given buffer, returning size of packet
func (m *Synthesis) RecvBuffer(buf []byte) (int, error) {
	if m.iprwc == nil {
		return 0, nil
	}
	n, _ := m.iprwc.Read(buf)
...
// Stop the mobile Synthesis instance
func (m *Synthesis) Stop() error {
	logger := log.New(m.log, "", 0)
	logger.EnableLevel("info")
	logger.Infof("Stop the mobile Synthesis instance %s", "")
...
func (m *Synthesis) RetryPeersNow() {
...
func (m *Synthesis) GetDHTJSON() (result string) {
