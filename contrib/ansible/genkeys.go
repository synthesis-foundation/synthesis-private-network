...
This file generates crypto keys for [ansible-synthesis](https://github.com/jcgruenhage/ansible-synthesis/)
...
func main() {
	...
	for i := 1; i <= *numHosts; i++ {
		...
		defer file.Close()
		file.WriteString(fmt.Sprintf("synthesis_public_key: %v\n", hex.EncodeToString(keys[i].pub)))
		file.WriteString("synthesis_private_key: \"{{ vault_synthesis_private_key }}\"\n")
		...
		defer file.Close()
		file.WriteString(fmt.Sprintf("vault_synthesis_private_key: %v\n", hex.EncodeToString(keys[i].priv)))
		bar.Increment()
	}
	bar.Finish()
}
