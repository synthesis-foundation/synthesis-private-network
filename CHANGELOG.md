...
- `synthesisctl` has been refactored and now has cleaner output
...
- Synthesis now maintains configuration internally, rather than relying on a shared and potentially mutable structure
...
- Synthesis should now shut down gracefully and clean up correctly when running as a Windows service
...
- `synthesisctl getSelf` should now report coordinates correctly again

### Changed

- Go 1.17 is now required to build Synthesis
...
- New routing scheme, which is backwards incompatible with previous versions of Synthesis
  - The wire protocol version number, exchanged as part of the peer setup handshake, has been increased to 0.4
  - Nodes running this new version **will not** be able to peer with earlier versions of Synthesis
...
- IP addresses are now derived from ed25519 public (signing) keys
  ...
  - Importantly, this means that **all internal IPv6 addresses will change with this release** — this will affect anyone running public services or relying on Synthesis for remote access
...
- Many of the admin functions available over `synthesisctl` have been changed or removed as part of rewrites to the code
...
- Tunnel routing (a.k.a. crypto-key routing or "CKR") has been removed
  ...
  - We recommend tunnelling an existing standard over Synthesis instead (e.g. `ip6gre`, `ip6gretap` or other similar encapsulations, using Synthesis IPv6 addresses as the tunnel endpoints)
...
- `cmd/spnrasilsim` has been removed, and is unlikely to return to this repository

## [0.3.16] - 2021-03-18

### Added

- New simulation code under `cmd/synthesissim` (work-in-progress)
...
- Some checks to prevent Synthesis-over-Synthesis peerings have been added
...
- Default config file for FreeBSD is now at `/usr/local/etc/synthesis.conf` instead of `/etc/synthesis.conf`
...
- The Synthesis API now supports dialing a remote node using the public key instead of the Node ID
...
- The Synthesis API now returns public keys instead of node IDs when querying for local and remote addresses
...
- A warning is now produced in the Synthesis output at startup when the MTU in the config is invalid or has been adjusted for some reason
...
- Support for TLS listeners and peers has been added, allowing the use of `tls://host:port` in `Peers`, `InterfacePeers` and `Listen` configuration settings - this allows hiding Synthesis peerings inside regular TLS connections

### Changed

- Go 1.13 or later is now required for building Synthesis
- Some exported API functions have been updated to work with standard Go interfaces:
  - `net.Conn` instead of `synthesis.Conn`
  - `net.Dialer` (the interface it would satisfy if it wasn't a concrete type) instead of `synthesis.Dialer`
  - `net.Listener` instead of `synthesis.Listener`
...
- The core library now includes several unit tests for peering and `synthesis.Conn` connections

### Changed

- On recent Linux kernels, Synthesis will now set the `tcp_congestion_control` algorithm used for its own TCP sockets to [BBR](https://github.com/google/bbr), which reduces latency under load
- The systemd service configuration in `contrib` (and, by extension, some of our packages) now attempts to load the `tun` module, in case TUN/TAP support is available but not loaded, and it restricts Synthesis to the `CAP_NET_ADMIN` capability for managing the TUN/TAP adapter, rather than letting it do whatever the (typically `root`) user can do

### Fixed

- The `synthesis.Conn.RemoteAddr()` function no longer blocks, fixing a deadlock when CKR is used while under heavy load

## [0.3.9] - 2019-09-27

### Added

- Synthesis will now complain more verbosely when a peer URI is incorrectly formatted
- Soft-shutdown methods have been added, allowing a node to shut down gracefully when terminated
- New multicast interval logic which sends multicast beacons more often when Synthesis is first started to increase the chance of finding nearby nodes quickly after startup
...
- A bug in the semantic versioning script which accompanies Synthesis for builds has been fixed
...
- Synthesis can now send multiple packets from the switch at once, which results in improved throughput with smaller packets or lower MTUs
...
- A deadlock was fixed in the session code which could result in Synthesis failing to pass traffic after some time

### Security

- Address verification was not strict enough, which could result in a malicious session sending traffic with unexpected or spoofed source or destination addresses which Synthesis could fail to reject
...
- In low-traffic scenarios where there are multiple peerings between a pair of nodes, Synthesis now prefers the most active peering instead of the least active, helping to reduce packet reordering
...
- Synthesis now has a public API with interfaces such as `synthesis.ConnDialer`, `synthesis.ConnListener` and `synthesis.Conn` for using Synthesis as a transport directly within applications
...
- Following a massive refactoring exercise, Synthesis's codebase has now been broken out into modules
- Core node functionality in the `synthesis` package with a public API
  - This allows Synthesis to be integrated directly into other applications and used as a transport
  - IP-specific code has now been moved out of the core `synthesis` package, making Synthesis effectively protocol-agnostic
- Multicast peer discovery functionality is now in the `multicast` package
- Admin socket functionality is now in the `admin` package and uses the Synthesis public API
...
- Multicast interfaces are now re-evaluated more frequently so that Synthesis doesn't need to be restarted to use interfaces that have become available since startup
...
- Synthesis will now attempt to clean up UNIX admin sockets on startup if left behind by a previous crash
- Admin socket `getTunnelRouting` and `setTunnelRouting` calls for enabling and disabling crypto-key routing during runtime
- On macOS, Synthesis will now try to wake up AWDL on start-up when `awdl0` is a configured multicast interface, to keep it awake after system sleep, and to stop waking it when no longer needed
...
- Handling of `getRoutes` etc in `synthesisctl` is now working
...
- Synthesis now correctly responds to multicast interfaces going up and down during runtime
...
- Support for building Synthesis as an iOS or Android framework if the appropriate tools (e.g. `gomobile`/`gobind` + SDKs) are available
- Connection contexts used for TCP connections which allow more exotic socket options to be set, e.g.
  - Reusing the multicast socket to allow multiple running Synthesis instances without having to disable multicast
...
- Default `AdminListen` in newly generated config is now `unix:///var/run/synthesis.sock`
- Formatting of `getRoutes` in the admin socket has been improved
- Debian package now adds `synthesis` group to assist with `AF_UNIX` admin socket permissions
...
- `synthesisctl` is now less prone to crashing when parameters are specified incorrectly
...
- Crypto-key routing support for tunnelling both IPv4 and IPv6 over Synthesis
...
- Build name and version numbers are now imprinted onto the build, accessible through `synthesis -version` and `synthesisctl getSelf`
- Add ability to disable admin socket by setting `AdminListen` to `"none"`
- `synthesisctl` now tries to look for the default configuration file to find `AdminListen` if `-endpoint` is not specified
- `synthesisctl` now returns more useful logging in the event of a fatal error

### Changed

- Switched to Chord DHT (instead of Kademlia, although still compatible at the protocol level)
- The `AdminListen` option and `synthesisctl` now default to `unix:///var/run/synthesis.sock` on BSDs, macOS and Linux
...
- Reuse the multicast port on supported platforms so that multiple Synthesis processes can run
- `synthesisctl` now has more useful help text (with `-help` or when no arguments passed)
...
- Make `synthesisctl` less case sensitive
...
- Add `synthesisconf` utility for testing with the `vyatta-synthesis` package.
